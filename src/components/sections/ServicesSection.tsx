      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((service, i) => (
            <motion.div key={service.id} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.08, duration: 0.5 }} whileHover={{ y: -3, transition: { duration: 0.2 } }} className="group bg-white rounded-2xl border border-slate-100 shadow-[0_8px_40px_rgba(0,0,0,0.13)] hover:shadow-[0_20px_60px_rgba(0,0,0,0.22)] hover:border-slate-200 transition-all duration-300 overflow-hidden">
