    ...
    <section id="projecten" className="py-24 px-6 bg-[#0D1117]" ref={ref}>
      <div className="max-w-7xl mx-auto">
            ...
            <motion.div key={project.id} initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1, duration: 0.6, ease: "easeOut" }} whileHover={{ y: -4, transition: { duration: 0.2 } }} className="group relative bg-[#0F1623] rounded-2xl border border-[#1A2332] p-7 hover:border-[#243447] transition-all duration-300 cursor-pointer overflow-hidden">
              ...
              {'video' in project && project.video ? (
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl" style={{ background: `${project.color}15`, border: `1px solid ${project.color}30` }}>{project.emoji}</div>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                {'url' in project && project.url && (
                  <a href={project.url as string} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-lg border transition-all" style={{ color: project.color, borderColor: `${project.color}40`, background: `${project.color}0D` }} onClick={(e) => e.stopPropagation()}>
                    <ExternalLink size={13} />{tr.visit}
                  </a>
                )}
                {'github' in project && project.github && (
                  <a href={project.github as string} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-lg border border-[#1A2332] bg-[#0F1623] text-[#8899B0] hover:text-[#E8EDF5] hover:border-[#243447] transition-all" onClick={(e) => e.stopPropagation()}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.5, duration: 0.5 }} className="text-center mt-10">
          <Link href="/projecten" className="group inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-[#1A2332] text-[#8899B0] text-sm font-medium hover:text-[#E8EDF5] hover:border-[#243447] hover:bg-[#0F1623] transition-all">
